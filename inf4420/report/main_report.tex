...
\begin{tikzpicture}[node distance=1cm,>=latex,
  ...
  \node [block, below left=2cm and -2.95cm of comparator](dac){DAC};
  \node [block, right= of samplehold](comparator){Comparator};
  \node [block, right= of comparator](sarlogic){SAR logic\\block};
  \node [block, below right=1.845cm and 1cm of sarlogic](outputbuffer){Output Buffer};
  
  \begin{scope}[very thick]
  \draw [draw] (vin) -- node[name=vin]{$v_{in}$}(srlatch);
