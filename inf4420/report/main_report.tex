...
\begin{tikzpicture}[node distance=1cm,>=latex,
  ...
  \node [block] (srlatch) {SR Latch};
  \node [block, right= of srlatch](samplehold){Sample and hold};
  \node [block, below= of samplehold](dac){DAC};
  \node [block, right= of samplehold](comparator){Comparator};
  \node [block, right= of comparator](sarlogic){SAR logic\\block};
  \node [block, below= of sarlogic](outputbuffer){Output Buffer};
